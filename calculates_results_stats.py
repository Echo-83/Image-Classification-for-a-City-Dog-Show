...
def calculates_results_stats(results_dic):
    ...
    for i in results_dic:
        # check if the image is dog based on the element before last in the results_dic 
        if results_dic[i][-2] == 1:
                n_dogs_img += 1
        else:
                n_notdogs_img += 1
        # check if the classifier identified the element the same as the actual thing
        if results_dic[i][-3] == 1:
                n_match += 1
        # check if both actual and model have the same idea about it being a dog
        if results_dic[i][-2] == results_dic[i][-1]:
                # if any of them is equal to 1 it means it is a dog and it is correct
                if results_dic[i][-2] ==1:
                        n_correct_dogs += 1
                        # this inner if checks if both the classifier and the actual are the same 
                        if results_dic[i][-3] == 1: 
                                n_correct_breed +=1
                # else both of them said it is not a dog then yes it is correctly not a dog
                else:
                        n_correct_notdogs += 1 
    # pct_match - percentage of correct matches
